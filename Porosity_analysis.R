# ============================================================
#  Bony Endplate Porosity — Statistical Analysis
...
# ── 0. Load packages ───────────────────────────────
library(readxl)
library(lme4)       # report this as the package to fir the model
...
setwd("C:/Users/Ahmad Alminnawi/Desktop/pipeline_publication/modeling-part/submitted_version/second submission/media-guidlines-rev/media/prism/porosity")
df <- read_excel("porosity_R.xlsx", sheet = "Sheet1")

dep_variable <- "Porosity"
...
#writeLines(capture.output(shapiro.test(resid(model))), "analysis/1-Shapiro-Wilk_Normality_test.txt")
...
p1 <- ggplot(df, aes(x = Zone, y = Value)) +
  ...
  coord_cartesian(ylim = c(0, 1)) +    # set y-axis limits without removing points
  theme_classic() +
...
# Panel A: Raw data (percentage)
df$Value_percent <- df$Value * 100
p1 <- ggplot(df, aes(x = Zone, y = Value_percent)) +
  geom_boxplot(outlier.shape = NA, fill = "lightgray") +
  geom_quasirandom(width = 0.2, alpha = 0.7, color = "blue") +
  stat_summary(fun = mean, geom = "point", size = 3, color = "red") +
  coord_cartesian(ylim = c(0, 100)) +    # y-axis from 0 to 100
  theme_classic() +
  labs(title = "A. Raw Data", y = paste0(dep_variable, "(%)"), x = "Zone") +
  theme(plot.title = element_text(face = "bold", size = 12))
# Save the plot
ggsave(filename = "graphs/panelA_raw_data_percent.png", plot = p1,
       width = 6, height = 4, dpi = 300)
