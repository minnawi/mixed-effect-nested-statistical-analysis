# ============================================================
# Bony Endplate Radii — MANUAL Power Analysis
# ============================================================
...
# 1. User settings
# -----------------------------
setwd("C:/Users/Ahmad Alminnawi/Desktop/pipeline_publication/modeling-part/submitted_version/second submission/media-guidlines-rev/media/prism/pore_diameter/power_analysis")

file_path    <- "Radii_R.xlsx"
sheet_name   <- "Sheet1"
response_var <- "Value"      # change to "Radii" if needed
cow_var      <- "Cow"
vertebra_var <- "Vertebra"
zone_var     <- "Zone"

output_dir <- "power_Radii_manual"
...
df <- df %>% filter(!is.na(Cow), !is.na(Vertebra), !is.na(Zone), !is.na(Radii))
...
cat("========== Radii MANUAL POWER ANALYSIS ==========\n\n")
...
pilot_reml <- lme4::lmer(
  Radii ~ Zone + (1 | Cow) + (1 | CowVertebra),
  data = df,
  REML = TRUE,
...
simulate_radii <- function(dat, beta, sd_cow, sd_cv, sd_resid) {
...
one_power_run <- function(dat, beta, sd_cow, sd_cv, sd_resid, alpha = 0.05) {
  dat <- dat
  dat$y_sim <- simulate_radii(dat, beta, sd_cow, sd_cv, sd_resid)
...
plot(curve_cows$n_cows, curve_cows$power,
     ...
     main = "Manual power curve for radii by number of cows")
...
# -----------------------------
cow_grid  <- c(2, 4, 6, 8, 10, 12, 14, 16)
