# ============================================================
# Bony Endplate Porosity — MANUAL Power Analysis
# ============================================================
...
# 1. User settings
# -----------------------------
file_path    <- "porosity_R.xlsx"
sheet_name   <- "Sheet1"
response_var <- "Value"      # change to "Porosity" if needed
cow_var      <- "Cow"
vertebra_var <- "Vertebra"
zone_var     <- "Zone"

output_dir <- "power_porosity_manual"
...
df <- df %>% filter(!is.na(Cow), !is.na(Vertebra), !is.na(Zone), !is.na(Porosity))
...
cat("========== POROSITY MANUAL POWER ANALYSIS ==========\n\n")
...
pilot_reml <- lme4::lmer(
  Porosity ~ Zone + (1 | Cow) + (1 | CowVertebra),
  data = df,
  REML = TRUE,
...
simulate_porosity <- function(dat, beta, sd_cow, sd_cv, sd_resid) {
...
one_power_run <- function(dat, beta, sd_cow, sd_cv, sd_resid, alpha = 0.05) {
  dat <- dat
  dat$y_sim <- simulate_porosity(dat, beta, sd_cow, sd_cv, sd_resid)
...
plot(curve_cows$n_cows, curve_cows$power,
     ...
     main = "Manual power curve for porosity by number of cows")
